  ...
  try {
    if (slug === 'america-spain-activity') {
      ...
      console.log('Notion記事を取得しました:', post?.title)
               ...
               slug.includes('america') || 
               slug.includes('spain') ||
               slug.match(/^[a-f0-9-]{36}$/)) { // UUID形式のNotionページID
      // アメリカ・スペイン活動記の各話を取得
      try {
        const episodes = await getAmericaSpainActivityPosts()
        // スラッグまたはページIDで検索
        post = episodes.find(episode => 
          episode.slug === slug || 
          episode.id === slug ||
          episode.slug.includes(slug)
        )
        console.log('エピソード記事を取得しました:', post?.title)
      } catch (error) {
        console.log('Notion episodes取得エラー:', error)
      }
    }
  } catch (error) {
